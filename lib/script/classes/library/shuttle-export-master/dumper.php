...
//		}
        ...
        protected function table_index($table) {
        $eol = $this->eol;
        $this->dump_file->write("-- $eol--Indexes for table `$table` $eol-- $eol");
        $this->dump_file->write("ALTER TABLE `$table`$eol");
        $index_table_sql = $this->get_index_table_sql($table);
        if (count($index_table_sql)) {
            $loop = 0;
            foreach ($index_table_sql as $index) {
                $key = $index["Key_name"];
                $col = $index["Column_name"];
                $subpart = "";
                $lastColn = ",".$eol;
                $loop++;
                if($loop == count($index_table_sql))
                    $lastColn =";".$eol;
                if (isset($index["Sub_part"]) && !empty($index["Sub_part"]))
                    $subpart = "(".$index["Sub_part"].")";
                
                if($key === "PRIMARY")
                    $this->dump_file->write(" ADD $key KEY (`$col`$subpart)$lastColn");
                else 
                    $this->dump_file->write(" ADD KEY `$key` (`$col`$subpart)$lastColn");
            }
	...
	public function get_create_table_sql($table) {
        ...
        public function get_index_table_sql($table) {
		$index_table_sql = $this->db->fetch('SHOW INDEX FROM `' . $table . '`');
		return $index_table_sql;
	}
}
