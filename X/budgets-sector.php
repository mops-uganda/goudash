...
$data->table('view_sector_budgets');
...
// SELECT itembudget.Vote As VoteCode, votes.VoteName, SUM(`Q1Budget`) AS `Q1Budget`, SUM(`Q2Budget`) AS `Q2Budget`, SUM(`Q3Budget`) AS `Q3Budget`, SUM(`Q4Budget`) AS `Q4Budget`, SUM(`BudgetEstimates`) AS `AnnualBudget`, SUM(`BudgetEstimates`)*100/(SELECT SUM(BudgetEstimates) FROM itembudget) AS Percent FROM `itembudget` LEFT JOIN votes ON itembudget.Vote = votes.VoteCode GROUP BY itembudget.Vote
$data->unset_add();
$data->unset_edit();
$data->unset_remove();
...
$data->label(array('SectorCode' => 'Code','Q1Budget'=>'Q1 Budget','Q2Budget'=>'Q2 Budget','Q3Budget' => 'Q3 Budget','Q4Budget'=>'Q4 Budget','AnnualBudget'=>'Annual Budget','Percent'=>'%'));
